...
const WA        = (m) => 'https://wa.me/' + WA_NUM + '?text=' + encodeURIComponent(m);
...
export const metadata = {
  ...
  description: "Konyaaltı Hurma, Liman, Sarısu, Uncalı, Gürsu, Öğretmenevleri, Ahatlı, Toros mahallelerinde terzi ve dikiş atölyesi. Bay, bayan, çocuk kıyafet dikimi, üniforma üretimi, fason imalat. Antalya'nın tüm ilçelerine hizmet. ☎ " + PHONE,
  keywords: [
...
const jsonLd = {
  ...
  '@graph': [
    ...
    },
    {
      ...
      ].map(function(n){ return { '@type': 'Place', name: n }; }),
      hasOfferCatalog: {
        '@type': 'OfferCatalog',
...
const OTHER_DISTRICTS = ['Muratpaşa', 'Kepez', 'Döşemealtı', 'Aksu', 'Lara', 'Serik', 'Kemer', 'Alanya', 'Manavgat'];
...
const REPAIR_CATEGORIES = {
  tr: [
    { icon: '✂️', title: 'Dikim Hizmetleri', keyword: 'terzi dikim Antalya · özel ölçü dikim Konyaaltı', desc: 'Sıfırdan kıyafet dikimi. Bay, bayan, çocuk — ölçünüze özel tasarım ve üretim.', rows: [['Erkek Gömlek Dikimi', '₺350+'], ['Erkek Pantolon Dikimi', '₺400+'], ['Kadın Elbise Dikimi', '₺600+'], ['Abiye / Özel Gün Dikimi', '₺900+'], ['Çocuk Kıyafeti Dikimi', '₺250+']] },
    { icon: '🔧', title: 'Tamir Hizmetleri', keyword: 'kıyafet tamiri Antalya · fermuar tamiri Konyaaltı', desc: 'Yırtık, sökük, fermuar, düğme — günlük giyimde en sık ihtiyaç duyulan onarımlar.', rows: [['Fermuar Değişimi (Pantolon/Kot)', '₺200+'], ['Fermuar Değişimi (Mont/Ceket)', '₺300+'], ['Yırtık / Sökük Onarımı', '₺150+'], ['Düğme, Kopça, Fitil Tamiri', '₺60+'], ['Astar Değişimi', '₺300+']] },
    { icon: '📏', title: 'Tadilat Hizmetleri', keyword: 'paça kısaltma Antalya · bel daraltma Konyaaltı', desc: 'Kıyafetinizi vücudunuza tam oturtan hassas ölçü ayarları.', rows: [['Paça Kısaltma (Pantolon/Kot/Etek)', '₺150+'], ['Bel Daraltma', '₺150+'], ['Kol Kısaltma', '₺200+'], ['Elbise / Ceket Daraltma', '₺200+'], ['Gelinlik & Abiye Tadilatı', '₺400+']] },
    { icon: '🧺', title: 'Ütü & Kuru Temizleme', keyword: 'ütü hizmeti Antalya · kuru temizleme Konyaaltı', desc: 'Profesyonel buharlı ütü ve kuru temizleme. Otel ve adreslerden alım-teslimat.', rows: [['Ütü (adet)', '₺80+'], ['Kuru Temizleme (Elbise)', '₺300+'], ['Kuru Temizleme (Mont/Kaban)', '₺500+'], ['Çamaşır & Ütü (kg)', '₺80+/kg']] },
  ],
  en: [
    { icon: '✂️', title: 'Tailoring Services', keyword: 'tailor Antalya · custom-fit sewing Konyaaltı', desc: 'Made-from-scratch garments. Men, women, children — designed and sewn to your exact measurements.', rows: [["Men's Shirt Tailoring", '₺350+'], ["Men's Trousers Tailoring", '₺400+'], ["Women's Dress Making", '₺600+'], ['Evening Gown / Special Occasion', '₺900+'], ["Children's Clothing", '₺250+']] },
    { icon: '🔧', title: 'Repair Services', keyword: 'clothing repair Antalya · zipper repair Konyaaltı', desc: 'Tears, seams, zippers, buttons — the everyday fixes clothing needs most.', rows: [['Zipper Replacement (Trousers/Jeans)', '₺200+'], ['Zipper Replacement (Coat/Jacket)', '₺300+'], ['Tear / Seam Repair', '₺150+'], ['Button, Hook, Snap Repair', '₺60+'], ['Lining Replacement', '₺300+']] },
    { icon: '📏', title: 'Alterations', keyword: 'hemming Antalya · waist taking in Konyaaltı', desc: 'Precise fit adjustments so your clothing sits exactly right on your body.', rows: [['Hemming (Trousers/Jeans/Skirt)', '₺150+'], ['Waist Taking In', '₺150+'], ['Sleeve Shortening', '₺200+'], ['Dress / Jacket Taking In', '₺200+'], ['Wedding & Evening Dress Alterations', '₺400+']] },
    { icon: '🧺', title: 'Ironing & Dry Cleaning', keyword: 'ironing service Antalya · dry cleaning Konyaaltı', desc: 'Professional steam ironing and dry cleaning.', rows: [['Ironing (per item)', '₺80+'], ['Dry Cleaning (Dress)', '₺300+'], ['Dry Cleaning (Coat)', '₺500+'], ['Laundry & Ironing (per kg)', '₺80+/kg']] },
  ],
  ru: [
    { icon: '✂️', title: 'Пошив одежды', keyword: 'портной Анталья · пошив по меркам Коньяалты', desc: 'Пошив одежды с нуля. Мужская, женская, детская — по индивидуальным меркам.', rows: [['Пошив мужской рубашки', '₺350+'], ['Пошив мужских брюк', '₺400+'], ['Пошив женского платья', '₺600+'], ['Вечернее / праздничное платье', '₺900+'], ['Пошив детской одежды', '₺250+']] },
    { icon: '🔧', title: 'Ремонт одежды', keyword: 'ремонт одежды Анталья · ремонт молнии Коньяалты', desc: 'Разрывы, швы, молнии, пуговицы — самый частый ремонт повседневной одежды.', rows: [['Замена молнии (брюки/джинсы)', '₺200+'], ['Замена молнии (пальто/куртка)', '₺300+'], ['Ремонт разрыва / шва', '₺150+'], ['Ремонт пуговиц, крючков, кнопок', '₺60+'], ['Замена подкладки', '₺300+']] },
    { icon: '📏', title: 'Подгонка одежды', keyword: 'укорачивание брюк Анталья · заужение талии Коньяалты', desc: 'Точная подгонка по фигуре — чтобы одежда сидела идеально.', rows: [['Укорачивание (брюки/джинсы/юбка)', '₺150+'], ['Заужение талии', '₺150+'], ['Укорачивание рукавов', '₺200+'], ['Заужение платья / пиджака', '₺200+'], ['Подгонка свадебного/вечернего платья', '₺400+']] },
    { icon: '🧺', title: 'Глажка и химчистка', keyword: 'глажка Анталья · химчистка Коньяалты', desc: 'Профессиональная паровая глажка и химчистка.', rows: [['Глажка (за вещь)', '₺80+'], ['Химчистка (платье)', '₺300+'], ['Химчистка (пальто)', '₺500+'], ['Стирка и глажка (за кг)', '₺80+/кг']] },
  ],
};
        ...
        <main id="main-content">
          ...
          <section className="sec" aria-labelledby="nb-h">
            <div className="ctr">
              ...
              <ul className="nb-grid" aria-label="Öncelikli hizmet mahalleleri">
                {NEIGHBORHOODS.map(function(n){ return (
              ...
              <ul className="other-districts" aria-label="Ek hizmet ilçeleri">
                {OTHER_DISTRICTS.map(function(d){ return (<li key={d} className="od-chip">{d}</li>); })}
              </ul>
            </div>
          </section>
          ...
          <section className="sec" style={{ background: 'rgba(0,0,0,.12)' }} id="workshop" aria-labelledby="wk-h">
            <div className="ctr">
              ...
              <div className="wk-grid">
                {WORKSHOP_SERVICES.map(function(s){ return (
                  <article className="wk-card" key={s.tr}>
                    ...
                    <ul className="wk-items" aria-label={s.tr + ' kapsamı'}>
                      {s.items.map(function(i){ return <li key={i} className="wk-item">{i}</li>; })}
                    </ul>
                  </article>
                );})}
              </div>
          ...
          <section className="sec" id="fiyatlar" aria-labelledby="price-h">
            <div className="ctr">
              ...
              {Object.keys(REPAIR_CATEGORIES).map(function(lang){ return (
                <div key={lang} style={{ marginBottom: lang !== 'ru' ? '3rem' : 0 }}>
                  ...
                  <div className="price-grid">
                    {REPAIR_CATEGORIES[lang].map(function(cat){ return (
                      <article className="price-card" key={cat.title}>
                        ...
                        <table className="price-table" aria-label={cat.title + ' — ' + LANG_LABELS[lang].name}>
                          ...
                          <tbody>
                            {cat.rows.map(function(row){ return (<tr key={row[0]}><td>{row[0]}</td><td>{row[1]}</td></tr>); })}
                          </tbody>
                        </table>
                      </article>
                    );})}
