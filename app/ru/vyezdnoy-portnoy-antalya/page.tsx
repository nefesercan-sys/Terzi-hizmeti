...
export const metadata: Metadata = {
  ...
  title: 'Выездной портной Анталья — Все отельные районы',
  description: 'Выездной портной в отели курортных районов Антальи: Белек, Лара, Гюзельоба, Кемер, Сиде. Выберите район, укажите название отеля.',
  alternates: {
      ...
      <section className="sec" aria-labelledby="hub-h">
        <div className="ctr" style={{ maxWidth: 760 }}>
          <div className="sec-head">
            <span className="eyebrow">Портной в отеле</span>
            <h1 className="sec-h ff" id="hub-h">Выездной портной в отельных районах Антальи</h1>
            <p className="sec-sub">Выберите район — напишите название отеля и даты в WhatsApp, наш портной приедет к вам.</p>
          </div>
          <div className="nb-grid">
            {OTEL_BOLGELERI.map((r) => (
              <a key={r.slug} href={`${BASE_PATH}/${r.slug}`} className="nb-card">
                <span className="nb-name">{r.name}</span>
                <span className="nb-tag">{r.travelTime.ru}</span>
              </a>
            ))}
          </div>
        </div>
